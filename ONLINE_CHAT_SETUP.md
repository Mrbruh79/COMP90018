...
4. On first launch, tap Continue to setup, then sign in with Email or Google. Email sign-up asks for a unique username and a display name. A first-time Google sign-in, or an existing account without a username, gets a one-time profile prompt. Existing users can link another sign-in method in Settings. Signing out hides local chats and contacts until that account signs in again. There is no anonymous mode. Phone numbers are contact details, not a sign-in method.
...
The cloud chat route uses Firebase account IDs. Signed-in users publish a small account card with their unique lowercase username, display name, and app peer ID. Usernames use 3 to 20 letters, numbers or underscores. Display names may repeat. Firestore reserves usernames transactionally and checks the reservation when an account card is written. Their verified sign-in email is available for exact lookup. A number becomes searchable only when its owner enters it under Settings > Find me, checks the phone-lookup box, and saves. A number printed on the QR card is separate and does not enable online lookup. Existing checked numbers move into the new discovery field when the app updates. That number is not verified, and any account could claim it. A saved contact can be resolved by QR or Nearby identity, verified account email, or an opted-in number, in that order. Online direct and private group messages use the same local message IDs as Nearby delivery.
...
- Country-code and number fields are normalized to one international format before lookup. For example, country code `61` plus `0412 345 678` matches `+61412345678`. A saved contact has a Find online action to check the account match and retry pending messages. Both people must use the updated app; the person being found must save Settings > Find me with lookup enabled while online.
...
- Chats, contacts, profile details, and local event data are stored in files scoped to the signed-in account on this phone. Signing out does not delete them, but it hides them until that account signs back in. The old unscoped database is assigned to the account already signed in during the upgrade. If the app is signed out during the upgrade, the old database is not assigned to a later login because its owner cannot be established safely.
- Username reservations cannot be transferred or renamed in this version. A malicious client can reserve unused names without finishing sign-up; a trusted server and abuse controls are needed before public rollout.
